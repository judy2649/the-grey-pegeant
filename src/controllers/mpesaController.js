...
exports.handleCallback = async (req, res) => {
    try {
        ...
        try {
            ...
            else console.log('✅ All callback notifications sent successfully');
    ...
    exports.submitManualVerification = async (req, res) => {
        try {
            ...
            try {
                ...
                const failed = results.filter(r => r.status === 'rejected' || (r.value && r.value.status === 'failed'));
                if (failed.length > 0) console.warn(`⚠️ Some manual notifications failed: ${failed.length}`);
                else console.log('✅ All manual verification notifications sent successfully');
